...
## Overview
Identifying key driver proteins capable of steering biological network state transitions with minimal energy consumption is a fundamental challenge in understanding cellular regulatory mechanisms. Existing methods, largely reliant on static network topology metrics, often overlook time-varying interactions and energy constraints, making it difficult to pinpoint energy-efficient driver proteins. To address this, we propose HyperDriver, a computational framework that characterizes time-varying interaction intensities through dynamic weight reconstruction and integrates static topology with high-order co-expression patterns using multi-scale hypergraph representations to build a unified dynamical state space. To enhance computational efficiency for large-scale networks, we introduce a spectral energy proxy to approximate average control energy, combined with iterative greedy search and adaptive clustering for the autonomous selection of key driver proteins. Experiments on 12 real-world yeast dynamic datasets demonstrate that the driver protein sets identified by HyperDriver reduce the control energy required for state transitions by 2–3 orders of magnitude. Furthermore, our findings reveal a systemic decoupling between structural hubs and energy hubs—proving that highly connected nodes are not necessarily optimal control targets—thereby providing a new perspective for drug target discovery based on energy efficiency optimization.
